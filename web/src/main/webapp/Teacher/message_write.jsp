...
<%
   ...
   String gr=(String)request.getParameter("gr");
   String stream=(String)request.getParameter("st");
   String spec=(String)request.getParameter("sp");
   String thread=(String)request.getParameter("th");
...
<div dojoType="dijit.Dialog" id="dialog_for_mess" title="Выбор адресата" execute="alert('submitted w/args:\n' + dojo.toJson(arguments[0], true));" onLoad="makeScrollPane()">
 ...
 <!--<div id="layout3" dojoType="dijit.layout.ContentPane" href="content_list_teachers.jsp"  title="Мои Коллеги" selected="true" refreshOnShow="true" onLoad="makeScrollPane('scrollPane3')">
    </div>-->
        ...
        <div class="otdel_blok_mes">
            <table class="tab_for_write_new_mes">
                <tr>
                    ...
                    <% }else if(gr!=null){%>
                        <div id="<%=gr %>">Группа <%=gr%> </div>
                        <input type="hidden" id="gr" name="gr" value='<%=gr %>' />
                          <input type="hidden" id="spec" name="spec" value='<%=spec %>' />
                    <% }else if((spec!=null)&&(thread==null) ){%>
                        <div id="<%=gr %>">Специальность  <%=spI.getName()%> </div>
                        <input type="hidden" id="spec" name="spec" value='<%=spI.getId()%>' />
                    <% }else if(thread!=null){%>
                        <div id="<%=gr %>">Специальность  <%=spI.getName()%> курс  <%=thread%> </div>
                       <input type="hidden" id="spec" name="spec" value='<%=spI.getId()%>' />
