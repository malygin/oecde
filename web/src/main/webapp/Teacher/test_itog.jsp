...
<jsp:useBean id="test" class="test.model.Test"/>
 ...
 <c:set var="umk" value="${testI.umk}"/>
 <c:set target="${test.testI}"  property="umk"  value="${param[\"umk\"]}"/>
...
<c:if test="${testI.title != null && param[\"id\"] != null }">
 <c:set var="questList" value="${test.controlQuestions}"/>
<script>
...
<div class=head_main_math>
    <p style="float: left; max-width: 350px;">
        <c:out value="${umk}" />
    </p>
    ...
    <form method="POST" id="info" name="info"  dojotype="dijit.form.Form">
        <table class="form_table_test">            
             <tr><td><br></td></tr>
            <tr>
                <td>
                    Название:*
                ...
                <td>
                    ...
                    <textarea dojoType="dijit.form.Textarea" name="testname" style="width:350px"><c:out value="${testI.title}"/></textarea>
                </td>
            </tr>
            <tr><td><br></td></tr>
            <tr>
                ...
                <td>
                    <c:choose>
                        <c:when test="${testI.type eq 'concluding'}">
                            <input dojoType="dijit.form.RadioButton" id="type1" name="type"
                               checked="checked" value="2" type="radio" />
                               <label for="type2"> Итоговый </label>
            ...
            <tr><td><br></td></tr>
            <tr>
                <td>Способ формирования*</td>
                <td><select id="use_quest" name="use_quest" dojoType="dojox.form.DropDownSelect">
                     <c:choose>
                        <c:when test="${testI.use_question}">
                           <option value="1" selected>из вопросов других тестов</option>
                           <option value="0">преподавателем</option>
                        </c:when>                        
                        <c:otherwise>
                            <option value="1" >из вопросов других тестов</option>
                           <option value="0" selected>преподавателем</option>
                        </c:otherwise>
                    </c:choose>
                   </select>
                </td>
            ...
            <tr>
                ...
                <td>
                    <textarea dojoType="dijit.form.Textarea"  name="comment" style="width:350px"><c:out value="${testI.comment}"/></textarea>
                </td>
            </tr>
            <tr><td><br></td></tr>
            ...
            <tr><td></td>
            <td>
                <button id="saveInfo" dojoType="dijit.form.Button" type="submit" >Сохранить</button><button dojoType=dijit.form.Button type="reset">Отменить</button></td></tr>
        </table>
    </form>
...
<div id="rsloaddiv"><p id="resultload"></p></div>
    ...
    <div id="questionList"  class="questionList">
        ...
        <table cellpadding="0" cellspacing="0" class="questionList_t_border">
            <c:forEach items="${testI.questions}" var="question" begin="0" varStatus="count">
                ...
                <tr>
                    <td colspan="4" class="questionList_buttons">
                        <button dojoType="dijit.form.Button" type="submit">Сохранить</button>
                        <button dojoType=dijit.form.Button type="reset">Отменить</button>
                    </td>
                </tr>
        </table>
        </form>
    ...
    <div class="GrayTitle">
    <div dojoType="dijit.TitlePane" title="Показать вопросы из промежуточных тестов" open="false">
    <div id="questionList"  class="questionList">
        <form id="questControlForm" name="questControlForm" method="post"  dojotype="dijit.form.Form">
        <table cellpadding="0" cellspacing="0" class="questionList_t_border">
            <c:forEach items="${questList}" var="question" begin="0" varStatus="count">
                <c:choose>
                      <c:when test="${count.count % 2 == 0}">
                            <c:set var="rowStyle" scope="page" value="darck"/>
                      </c:when>
                ...
                <tr class="<c:out value="${rowStyle}"/>">
                    ...
                    <td class="questionList_tr_td_r_b">
                        <input dojoType="dijit.form.NumberSpinner"
                            ...
                            constraints="{min:0,max:10,places:0}"
                            name="weight<c:out value="${question.id}"/>"
                            id="weightControl<c:out value="${question.id}"/>">
                     </td>
                   <td class="questioList_tr_td_b">
                     <c:if test="${question.conclusion}">
                        <input type="checkbox" name="itog<c:out value="${question.id}"/>" id ="itogcontrol<c:out value="${question.id}"/>" checked>
                     </c:if>
                     <c:if test="${!question.conclusion}">
                        <input type="checkbox" name="itog<c:out value="${question.id}"/>" id ="itogcontrol<c:out value="${question.id}"/>" >
                     </c:if>
                </td>
                </tr>
            </c:forEach>
