...
<%
    ...
    TeacherItem teacher=(TeacherItem)session.getAttribute("teacherItem");
    List<MessageItem> msgs = null;

    //это препод
    message.setTypeUser("3");
    ...
    int count = message.getCountMsgs(Integer.parseInt(teacher.getId()));
    ...
    if(message.getTypeList().equals("out")|| message.getTypeList().equals("in")|| message.getTypeList().equals("in_student")|| message.getTypeList().equals("in_teacher")|| message.getTypeList().equals("in_admin")||message.getTypeList().equals("arch")){
        msgs = message.getListByPage(Integer.parseInt(teacher.getId()), currentPage, countOut);
    }
                        ...
                        <c:if test="${messg.typeList ne 'out'}">
                             <c:choose>
                                <c:when test="${message.studentSender ne null}">
                                    <img style="float: left; margin-right: 10px;" src='../loadPhoto?id=<c:out value="${message.studentSender.id}"/>&type=student&size=mini'>
                               </c:when>
            ...
            <tr>
                <td colspan="2">
                  <c:if test="${messg.typeList ne 'out'}">
                     <c:choose>
                        <c:when test="${message.studentSender ne null}">
                          <a class="msg_author" href="#student/id=${message.studentSender.id}"> <c:out value="${message.studentSender.surname}"/> &nbsp<c:out value="${message.studentSender.name}"/>. Группа - <c:out value="${message.studentSender.gr}"/></a>
                        </c:when>
                        <c:when test="${message.teacherSender ne null}">
...
<div class="mess_tools_butt grey_button" style="background:white">
    ...
    <div class="div_forArchDel_butt">
        <div style="float: left;"><a href="javascript:void(0)" class="tool_links_msg" onclick="action('arch',null)">Архивировать</a>&nbsp;|&nbsp;</div>
        <div style="float: left;"><a href="javascript:void(0)" class="tool_links_msg" onclick="action('del', null)">Удалить</a></div>
        <div class="otdelitel"></div>
    </div>
    <div class="otdelitel"></div>
</div>
...
<c:if test="${messg.typeList eq 'arch' }">
    ...
    <div class="mess_tools_butt grey_button" style="background:white">
        ...
        <div class="div_forArchDel_butt">
            <div style="float: left;"><a href="javascript:void(0)" class="tool_links_msg" onclick="action('arch',null)">Разархивировать</a>&nbsp;|&nbsp;</div>
            <div style="float: left;"><a href="javascript:void(0)" class="tool_links_msg" onclick="action('del',null)">Удалить</a></div>
            <div class="otdelitel"></div>
        </div>
        <div class="otdelitel"></div>
