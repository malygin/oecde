...
<html>
    ...
    <script type="text/javascript">		
       ...
       dojo.addOnLoad(function fhref(){
            dojo.connect(document.info,'onsubmit',function (e){ sendinfo(); return dojo.stopEvent(e);});
            dojo.connect(document.questForm,'onsubmit',function (e){ sendQuestions(); return dojo.stopEvent(e);});
            dojo.connect(document.questControlForm,'onsubmit',function (e){ sendControlQuestions(); return dojo.stopEvent(e);});
            var href;


           if("<%=request.getParameter("type")%>" == "itog"){
               href = "test_itog.jsp?id=<%=request.getParameter("id")!= null?request.getParameter("id").replaceAll("[^0-9]", ""):""%>&umk=<%=request.getParameter("umk")!= null? request.getParameter("umk").replaceAll("[^0-9]", ""): ""%>";
               dijit.byId("contentPane").setHref(href);
           }              
           else if("<%=request.getParameter("type")%>" == "time"){
               href = "test_info.jsp?id=<%=request.getParameter("id")!= null?request.getParameter("id").replaceAll("[^0-9]", ""):""%>&umk=<%=request.getParameter("umk")!= null? request.getParameter("umk").replaceAll("[^0-9]", ""): ""%>";
                dijit.byId("contentPane").setHref(href);
           }
           else{
               window.location = "umkpassport.jsp?umk=<%=request.getParameter("umk")!= null? request.getParameter("umk").replaceAll("[^0-9]", ""): ""%>";
           }
               
       })
    </script>
    <script type="text/javascript" src="../scripts/date_today.js" djConfig="parseOnLoad: true"></script>
    <script type="text/javascript" src="../scripts/testedit.js" djConfig="parseOnLoad: true"></script>
