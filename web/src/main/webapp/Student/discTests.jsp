...
<div>
    ...
    <span class="msg1" style="float: right; margin-top: 10px; margin-right: 10px;"><a href="#mat/s=${s?1:0}"><<<вернуться назад</a></span>
        ...
        <c:forEach items="${se.testsResultsByDisc}" var="res" varStatus="i">
            ...
            <tbody id="int_table<c:out value='${i.index}'/>" style="margin: 0px; display: none;">
                <c:forEach items="${res.value}" var="result" varStatus="iterator">
                    <tr>
                        <td class="td_otch_page1"><c:out value="${(result.type eq 2)?(\"Переэказменовка\"):(\"Попытка\")}"/> <c:out value="${(result.type ne 2)?iterator.count:''}"/></td>
