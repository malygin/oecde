	...
	d.addOnLoad(function(){
		...
		var switchPage = function(arr){
			...
			var c = d.connect(_in, "onEnd", function(){
				...
				if (arr[i]==arr[i+1])
				{
					//console.debug(i,i+1,n)
					d.attr(n, "style", "display: none;")
				} else {
					d.attr(n, "style", "display: inline;")
				}
					d.attr(n, "src", arr[i])
				});
			...
			var path = "../textbooks/"+dijit.byId("infopart").href.split("/")[2]+"/"
			var firstP = dojo.byId("me").src;
			
		d.xhrGet({ 
			// load a list of additional images from a url:
			url:path+"images.json", handleAs:"json",

			// handle the response data from the url:
			load: function(resp){
				// basic loading, then make an array of thumbnail urls:
				var items = resp.images, needed = [], npages = (items.length /k);
				console.debug(items.length /k)
				
				var thumbs = d.map(items, function(item){
					var thumb = item.src.replace(/\./, "t.");
					return path+"images/" + thumb;
				});
				dojox.image.preload(thumbs);
				dojo.byId("me").src = firstP;
				...
				{
					pages.push(other.slice(i*k, (i+1)*k)); 
				}
				/*
				pages.push(other.slice(0, k)); 
				pages.push(other.slice(k,2*k));
				pages.push(other.slice(2*k,3*k));
				pages.push(other.slice(3*k,4*k));*/
